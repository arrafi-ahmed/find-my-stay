...
module.exports ={
	...
	getStatPaidBook: (hostId)=>{
		return new Promise((resolve, reject)=>{
			var sql = "SELECT count(id) book from bookings where (status = 2 or status = 3) and host_id = $1;";
			db.getResult(sql, [hostId], (result)=>{
				if(result.length > 0){
					resolve(result[0]);
				}else{
	...
	searchBooking: (query)=>{
		return new Promise((resolve, reject)=>{
			var sql = "SELECT  ROUND(rating, 1) rrating, p.* FROM property p LEFT JOIN bookings b ON b.start_date < TO_DATE($1, 'YYYY-MM-DD') AND b.end_date > TO_DATE($2, 'YYYY-MM-DD') AND b.property_id = p.id WHERE (city ILIKE $3 OR country ILIKE $4 OR country ILIKE $4) AND type in ($5, $6) AND b.id IS NULL;";
